...
import com.am.cs12.commu.protocolSate.beiDou.DriverBD;
import com.am.cs12.commu.protocolSate.beiDou.$TXXX.Data_TXXX;
...
public class BeiDouReceiver {//北斗用户机用户接口4.0

	private static Logger log = LogManager.getLogger(BeiDouReceiver.class.getName()) ;
	...
	public void receiveData(String comId, String protocolName , byte[] data) {
		try{
			...
			DriverBD driver = (DriverBD)initProtocolDriver(pvo.driverName , protocolName) ;
			...
			}else if(action.has(ActionSate.receivedData)){//目前只实现北斗4.0接收数据解析这部分
				DataSate sateData = driver.getData() ;
				if(sateData == null){
					log.error("严重错误，从卫星协议驱动解析出的数据为空！") ;
					return ;
				}
				Data_TXXX dout = (Data_TXXX)sateData.getSubData() ;
				if(dout == null){
					log.error("严重错误，从卫星协议驱动解析出的数据为空！") ;
					return ;
				}
...
//			}
	...
	private static DriverSate initProtocolDriver(String clazz , String protocolName) throws IllegalArgumentException {
		try {
			...
			if (c == null) {
				throw new IllegalArgumentException("不能实例化北斗卫星通信协议驱动类" + clazz + "！");
			}
