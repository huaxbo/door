...
public class LocalServer {
	 ...
	 */
	public LocalContext start()
			throws IllegalArgumentException {
		LocalServerVO vo = ConfigCenter.instance().getLocalServerVO() ;
		
		LocalContext ctx = new LocalContext();
		ctx.setObject(LocalServerConstant.MINASERVERID, vo.id);
		...
		//编解码过滤器
		chain.addLast("protocol", new ProtocolCodecFilter(new LocalCodecFactory()));
		...
		LocalIoHandler handler = new LocalIoHandler();
		...
		try {
			...
			e.printStackTrace();
			System.out.println("本地网络通信服务(ID:" + vo.id + ")启动失败！");
			isException = true ;
		}finally{
			;
		}
		...
		if(!isException){
			System.out.println("本地网络通信服务(ID:" + vo.id + ")已经启动 ，端口为" + vo.port);
			return ctx;
		}else{
			return null ;
		}
